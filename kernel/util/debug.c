...
static char *DEBUG_NAME[] = {
  [DEBUG_TRACE] = BLU  "TRACE" COLOR_RESET,
  [DEBUG_INFO] = GRN "INFO   " COLOR_RESET,
  [DEBUG_WARNING] = YEL "WARNING" COLOR_RESET,
  [DEBUG_ERROR] = RED "ERROR  " COLOR_RESET,
  [DEBUG_FATAL] = REDB "FATAL" COLOR_RESET,
};
...
static int debug_vsprintf(enum debug_level level, const char *fmt, va_list args) {
  ...
  char *process_name = current_process? current_process->name : "";
