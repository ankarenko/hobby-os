...
bool create_kernel_stack(virtual_addr* kernel_stack) {
  ...
  if (!*kernel_stack) {
	...
	ret = (void*) (*kernel_stack + stack_size);
...
void idle_task() {
	...
	while(1) 
    __asm volatile ("pause"); //__asm volatile ("pause" ::: "memory");
}
...
bool scheduler_initialize(void) {
  ...
  /* register isr */
  old_scheduler_isr = getvect(IRQ0);
